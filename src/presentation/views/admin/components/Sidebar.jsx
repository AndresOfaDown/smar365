 ...
 */
// Función utilitaria para combinar clases condicionales
const cn = (...classes) => classes.filter(Boolean).join(" ");

export const Sidebar = () => {
  const [isOpen, setIsOpen] = useState(true);
  const [openKey, setOpenKey] = useState("usuario"); // valor por defecto
  ...
  return (
    <aside
      className={cn(
        ...
        isOpen ? "w-72" : "w-20",
        "relative bg-gradient-to-b from-indigo-50/60 via-white/50 to-purple-50/50",
    ...
    >
      ...
      <div className="flex-1 px-2 pb-4 overflow-y-auto scrollbar-thin scrollbar-thumb-indigo-300/40 dark:scrollbar-thumb-slate-700/60">
            ...
            <div key={sec.key} className="mb-2">
              <button
                ...
                className={cn(
                  ...
                  "bg-white/70 dark:bg-slate-800/70 hover:bg-white/90 dark:hover:bg-slate-800",
                  "border border-indigo-100/70 dark:border-slate-700 transition"
                )}
              >
        ...
        <button
          ...
          className={cn(
            "mt-3 w-full rounded-xl px-3 py-2 text-left bg-white/70 dark:bg-slate-800/70",
            "border border-indigo-100/70 dark:border-slate-700 hover:bg-white/90 dark:hover:bg-slate-800",
        ...
        <Link
          ...
          className={cn(
            "mt-3 w-full rounded-xl px-3 py-2 text-left bg-white/70 dark:bg-slate-800/70",
            "border border-indigo-100/70 dark:border-slate-700 hover:bg-white/90 dark:hover:bg-slate-800",
            "transition text-sm font-medium text-blue-600 dark:text-blue-400"
          )}
        >
          Ir a SmartSales365
        </Link>
